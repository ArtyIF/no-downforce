...
func load_demo() -> void:
	...
	_car.reset()
	if not demo:
		NoDownforceGlobal.ui_manager.hide_screen("DemoScreen")
		NoDownforceGlobal.playing_demo = false
		return
	if demo.version == "" or demo.version == "0.2" or demo.version == "0.3":
		NoDownforceGlobal.ui_manager.get_screen(".").get_node("ErrorDialog").dialog_text = "The version of this demo is too old to be played properly."
		NoDownforceGlobal.ui_manager.get_screen(".").get_node("ErrorDialog").popup_centered()
		demo = null
		return
	if demo.version != ProjectSettings.get_setting("application/config/version"):
		NoDownforceGlobal.ui_manager.get_screen("DemoScreen").get_node("OutdatedDemoWarning").visible = true
		NoDownforceGlobal.ui_manager.get_screen("DemoScreen").get_node("OutdatedDemoWarning/VBox/Version").text = demo.version if demo.version != "" else "0.1"
	else:
		NoDownforceGlobal.ui_manager.get_screen("DemoScreen").get_node("OutdatedDemoWarning").visible = false
	NoDownforceGlobal.ui_manager.show_screen("DemoScreen", false)
