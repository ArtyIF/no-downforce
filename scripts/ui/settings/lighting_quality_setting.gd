...
@export_file("*.lmbake") var static_light_data: String
@export_file("*.lmbake") var dynamic_light_data: String
...
func on_value_changed(index: int):
	match index:
		0:
			$"/root/RaceTrack/LightmapGI".light_data = load(static_light_data)
			$"/root/RaceTrack/Lighting/Sun".light_bake_mode = Light3D.BAKE_STATIC
		1:
			$"/root/RaceTrack/LightmapGI".light_data = load(dynamic_light_data)
